...
void DWIN_G29_Show_Messge(const _emDWIN_G29_MSG message/*=G29_LEVLE_DEFAULT*/,const int pt_index,const int all_points,const float fvalue){
	...
	switch(message){
		...
		case G29_MESH_PROBING:
			...
			DWIN_Draw_Small_Float22(12+25*8, 454, probe.offset.z);
			break;
		case G29_MESH_VALUE:
		#if (GRID_MAX_POINTS_X <= 7)
...
void Draw_Prepare_Menu() {
	...
	const int16_t pscrol = MROWS - DwinMenu_prepare.index;
	#define PSCROL(L) (pscrol + (L))
#else
	#define PSCROL(L) (L)
#endif
...
void Draw_Home_Menu() {
	...
	const int16_t hscrol = MROWS - DwinMenu_home.index;
	#define HSCROL(L) (hscrol + (L))
#else
	#define HSCROL(L) (L)
#endif
...
void Draw_Temperature_Menu() {
	...
	const int16_t tcscrol = MROWS - DwinMenu_temp.index;
	#define TCSCROL(L) (tcscrol + (L))
#else
	#define TCSCROL(L) (L)
#endif
	...
	if (TCVISI(0)) Draw_Back_First(DwinMenu_temp.now == 0);             		 // < Back 
	...
	if (DwinMenu_temp.now) Draw_Menu_Cursor(DwinMenu_temp.now);
}
...
void Draw_Move_Menu() {	
	...
	const int16_t mcscrol = MROWS - DwinMenu_move.index;
	#define MSCROL(L) (mcscrol + (L))
#else
	#define MSCROL(L) (L)
#endif
	...
	dwinLCD.JPG_CacheTo1(HMI_flag.language+1);
	if (MVISI(0)) Draw_Back_First(DwinMenu_move.now == 0);             // < Back
	...
	if (HMI_flag.ETempTooLow_flag) {
		if (encoder_diffState == ENCODER_DIFF_ENTER) {
			...
			HMI_Value.Move_X_scale = current_position.x * MINUNITMULT;
			 DWIN_Draw_Small_Float31(MENUVALUE_X, MBASE(1), HMI_Value.Move_X_scale);
			HMI_Value.Move_Y_scale = current_position.y * MINUNITMULT;
			 DWIN_Draw_Small_Float31(MENUVALUE_X, MBASE(2), HMI_Value.Move_Y_scale);
			HMI_Value.Move_Z_scale = current_position.z * MINUNITMULT;
			 DWIN_Draw_Small_Float31(MENUVALUE_X, MBASE(3), HMI_Value.Move_Z_scale);
			 DWIN_Draw_Small_Float31(MENUVALUE_X, MBASE(4), HMI_Value.Current_E_Scale[0]);
			 DWIN_Draw_Small_Float31(MENUVALUE_X, MBASE(5), HMI_Value.Current_E_Scale[1]);
			dwinLCD.UpdateLCD();
		}
		return;
	}
	...
	if (encoder_diffState == ENCODER_DIFF_CW) {
		...
		switch (DwinMenu_move.now) {
			...
			case AXISMOVE_CASE_EX1: // Extruder1
				...
				HMI_Value.Current_E_Scale[0] = HMI_Value.Last_E_Coordinate[0]*MINUNITMULT;
...
void Draw_Filament_Menu() {
	...
	dwinLCD.JPG_CacheTo1(HMI_flag.language+1);
	if(DwinMenu_filament.now <= MROWS) Draw_Back_First(DwinMenu_filament.now == FILAMENT_CASE_BACK);

	Item_Filament_Preheat(FILAMENT_CASE_PREHEAT);
	Item_Filament_Extruder(FILAMENT_CASE_EXTRUDER);
	Item_Filament_Purgelength(FILAMENT_CASE_PURGELENGTH);
	Item_Filament_Feedlength(FILAMENT_CASE_FEEDLENGTH);	
	Item_Filament_Pugre(FILAMENT_CASE_PURGE);
	//Item_Filament_Retract(FILAMENT_CASE_RETRACT);
	//Item_Filament_load(FILAMENT_CASE_LOAD);
	//Item_Filament_unload(FILAMENT_CASE_UNLOAD);
	
	if (DwinMenu_filament.now) Draw_Menu_Cursor(DwinMenu_filament.now);
}
...
inline void _show_too_cool(uint8_t t){
	DWIN_Show_Status_Message(COLOR_RED, PSTR("Nozzle too cool!"));
	set_status_bar_showtime(t);	
}
...
static void Dwin_filament_action(uint8_t action){
	...
	else if(HMI_flag.clean_status_delay > 0) { 
		_warning_beep();	
		return;
	}
#if ENABLED(MIXING_EXTRUDER)
	if(HMI_Value.load_extruder <= E_STEPPERS){
		MIXER_STEPPER_LOOP(i) mixer.color[NR_USER_VIRTUAL_TOOLS][i] = (HMI_Value.load_extruder == i+1) ? COLOR_A_MASK : 0;
		mixer.T(NR_USER_VIRTUAL_TOOLS);
		mux = 1;
	}
	else if(HMI_Value.load_extruder == E_STEPPERS+1){
		MIXER_STEPPER_LOOP(i) mixer.color[NR_USER_VIRTUAL_TOOLS][i] = COLOR_A_MASK;
		mixer.T(NR_USER_VIRTUAL_TOOLS);
		mux = E_STEPPERS;
	}
#endif
	if(action == FILAMENT_CASE_PURGE){
		...
		DWIN_Show_Status_Message(COLOR_WHITE, PSTR("Slowly loading, please wait..."));
		...
		t = HMI_Value.purgelength*mux/FILAMENT_CHANGE_SLOW_LOAD_FEEDRATE;
		DWIN_Show_Status_Message(COLOR_WHITE, PSTR("Slowly unloading, please wait..."));
		set_status_bar_showtime(t<2?2:t);
		planner.buffer_line(current_position, feedrate_mm_s, active_extruder);
		planner.synchronize();
		...
		DWIN_Show_Status_Message(COLOR_WHITE, PSTR("Quickly loading, please wait..."));
		set_status_bar_showtime(t<2?2:t);
		planner.buffer_line(current_position, feedrate_mm_s, active_extruder);
		planner.synchronize();
		HMI_flag.Is_retracted = false;
		...
		if(!HMI_flag.Is_purged){
			...
			DWIN_Show_Status_Message(COLOR_WHITE, PSTR("First purge a little..."));
			set_status_bar_showtime(t<2?2:t);
			planner.buffer_line(current_position, feedrate_mm_s, active_extruder);		
			planner.synchronize();
		}
		...
		DWIN_Show_Status_Message(COLOR_WHITE, PSTR("Quickly unloading, please wait..."));		
		set_status_bar_showtime(t<2?2:t);
		planner.buffer_line(current_position, feedrate_mm_s, active_extruder);
		planner.synchronize();		
	}
#if ENABLED(MIXING_EXTRUDER)
	mixer.selected_vtool = old_vtool;
	mixer.T(mixer.selected_vtool);
#else
...
void Draw_Leveling_Menu() {
	...
	const int16_t lscrol = MROWS - DwinMenu_leveling.index;	
	#define LSCROL(L) (lscrol + (L))
#else
	#define LSCROL(L) (L)
#endif
	...
	SERIAL_ECHOLNPAIR("DwinMenu_leveling.now = ", DwinMenu_leveling.now);
	SERIAL_ECHOLNPAIR("DwinMenu_leveling.index= ", DwinMenu_leveling.index);
	SERIAL_ECHOLNPAIR("HMI_flag.Leveling_Case_Total= ", HMI_flag.Leveling_Case_Total);
#endif
	...
	if(LVISI(LEVELING_CASE_BACK)) Draw_Back_First(DwinMenu_leveling.now == LEVELING_CASE_BACK);    	// < Back 
...
#endif 
	if (DwinMenu_leveling.now) Draw_Menu_Cursor(LSCROL(DwinMenu_leveling.now));
	Draw_Status_Area();
}
...
inline void Draw_Language_Menu() {
	...
	Draw_Back_First(DwinMenu_language.now == LANGUAGE_CASE_BACK);
	
	DWIN_Show_MultiLanguage_String(MTSTRING_LANGUAGE_EN, LBLX, MBASE(LANGUAGE_CASE_EN));
	Draw_Menu_Line(LANGUAGE_CASE_EN,ICON_EN);
	DWIN_Show_MultiLanguage_String(MTSTRING_LANGUAGE_SP, LBLX, MBASE(LANGUAGE_CASE_SP));
	Draw_Menu_Line(LANGUAGE_CASE_SP,ICON_SP);
	DWIN_Show_MultiLanguage_String(MTSTRING_LANGUAGE_RU, LBLX, MBASE(LANGUAGE_CASE_RU));
	Draw_Menu_Line(LANGUAGE_CASE_RU,ICON_RU);
	DWIN_Show_MultiLanguage_String(MTSTRING_LANGUAGE_FR, LBLX, MBASE(LANGUAGE_CASE_FR));
	Draw_Menu_Line(LANGUAGE_CASE_FR,ICON_FR);
	DWIN_Show_MultiLanguage_String(MTSTRING_LANGUAGE_PT, LBLX, MBASE(LANGUAGE_CASE_PT));
	Draw_Menu_Line(LANGUAGE_CASE_PT,ICON_PT);
	
	if(DwinMenu_language.now) Draw_Menu_Cursor(DwinMenu_language.now);
}
...
void HMI_Prepare() {
	...
	if (encoder_diffState == ENCODER_DIFF_CW) {
		...
		switch (DwinMenu_prepare.now) {
			case 0: // Back
				DwinMenu_main.set(MAIN_CASE_PREPARE);
				Draw_Main_Menu();
			break;

...
void Draw_Control_Menu() {
	...
	const int16_t cscrol = MROWS - DwinMenu_control.index;
	#define CCSCROL(L) (cscrol + (L))
#else
	#define CCSCROL(L) (L)
#endif
	#define CCVISI(L) WITHIN(CCSCROL(L), 0, MROWS)

	SERIAL_ECHOLNPAIR("cscrol = ", cscrol);
...
void Draw_Mixer_Menu() {	
	...
	dwinLCD.JPG_CacheTo1(HMI_flag.language+1);
	Draw_Back_First(DwinMenu_mixer.now == 0);
...
inline void Draw_Mixer_Manual_Menu() {
	...
	const int16_t mscroll = MROWS - DwinMenu_manualmix.index; // Scrolled-up lines
...
inline void Draw_Mixer_Gradient_Menu() {
	...
	const int16_t ascroll = MROWS - DwinMenu_GradientMix.index; // Scrolled-up lines
...
inline void Draw_Mixer_Random_Menu() {
	...
	const int16_t rscroll = MROWS - DwinMenu_RandomMix.index; // Scrolled-up lines
	...
	dwinLCD.JPG_CacheTo1(HMI_flag.language+1);
	if (RCVISI(0)) Draw_Back_First(DwinMenu_RandomMix.now == 0); // < Back

	//Z start
	DWIN_Show_MultiLanguage_String(RANDOM_MENU_START, LBLX, RCLINE(RANDOM_CASE_ZPOS_START));
	DWIN_Show_MultiLanguage_String(RANDOM_MENU_Z, LBLX+GET_ICON_X(START), RCLINE(RANDOM_CASE_ZPOS_START));
	...
	//Z END
	DWIN_Show_MultiLanguage_String(RANDOM_MENU_END, LBLX, RCLINE(RANDOM_CASE_ZPOS_END));
	DWIN_Show_MultiLanguage_String(RANDOM_MENU_Z, LBLX+GET_ICON_X(END), RCLINE(RANDOM_CASE_ZPOS_END));
...
void Draw_Config_Menu(const uint8_t MenuItem, const uint8_t MenuIndex) {	
	DwinMenuID = DWMENU_CONFIG;
	DwinMenu_configure.set(MenuItem);
	DwinMenu_configure.index = _MAX(MenuItem, MenuIndex);

#if CONFIG_CASE_TOTAL > MROWS
	const int16_t coscrol = MROWS - DwinMenu_configure.index;
	#define COSCROL(L) (coscrol + (L))
#else
	#define COSCROL(L) (L)
#endif
...
inline void Draw_Retract_Menu() {
	...
	//Item
	DWIN_Draw_MaskString_Default(LBLX, MBASE(RETRACT_CASE_AUTO), PSTR("Auto-Retract:"));
	...
	DWIN_Draw_MaskString_Default(LBLX, MBASE(RETRACT_CASE_RETRACT_MM), PSTR("Retract MM:"));
	...
	DWIN_Draw_MaskString_Default(LBLX, MBASE(RETRACT_CASE_RETRACT_V), PSTR("Retract V:"));
	...
	DWIN_Draw_MaskString_Default(LBLX, MBASE(RETRACT_CASE_RETRACT_ZHOP), PSTR("Retract ZHop:"));
	...
	DWIN_Draw_MaskString_Default(LBLX, MBASE(RETRACT_CASE_RECOVER_MM), PSTR("UnRetract MM:"));
	...
	//DWIN_Draw_MaskString_Default(LBLX, MBASE(RETRACT_CASE_RECOVER_V), PSTR("UnRetract V:"));
	...
	//Curor
	Draw_Back_First(DwinMenu_fwretract.now == 0);
	if (DwinMenu_fwretract.now) Draw_Menu_Cursor(DwinMenu_fwretract.now);
}
...
static void Draw_Motion_Menu(const uint8_t MenuItem = 0, const uint8_t MenuIndex = MROWS) {

	DwinMenuID = DWMENU_MOTION;
	DwinMenu_motion.set(MenuItem);
  DwinMenu_motion.index = _MAX(MenuItem, MenuIndex);
	...
	dwinLCD.JPG_CacheTo1(HMI_flag.language+1);
	Draw_Back_First(DwinMenu_motion.now == 0);
...
inline void Draw_Steps_Menu() {
	...
	dwinLCD.JPG_CacheTo1(HMI_flag.language+1);
	Draw_Back_First(DwinMenu_step.now == 0);
...
void HMI_Motion() {
	...
	// Avoid flicker by updating only the previous menu
	if (encoder_diffState == ENCODER_DIFF_CW) {
		if (DwinMenu_motion.inc(MOTION_CASE_END)) Move_Highlight(1, DwinMenu_motion.now);
	}
	else if (encoder_diffState == ENCODER_DIFF_CCW) {
		if (DwinMenu_motion.dec()) Move_Highlight(-1, DwinMenu_motion.now);
	}
	else if (encoder_diffState == ENCODER_DIFF_ENTER) {
...
void HMI_Config() {
 ...
 // Avoid flicker by updating only the previous menu
	if (encoder_diffState == ENCODER_DIFF_CW) {
		...
		else if(DwinMenu_configure.inc(1 + CONFIG_CASE_TOTAL)){
			if (DwinMenu_configure.now > MROWS && DwinMenu_configure.now > DwinMenu_configure.index) {
				DwinMenu_configure.index = DwinMenu_configure.now;
				// Scroll up and draw a blank bottom line
				Scroll_Menu(DWIN_SCROLL_UP);
...
void HMI_Control() {
	...
	if (encoder_diffState == ENCODER_DIFF_CW) {
		...
		switch (DwinMenu_control.now) {
			case 0: // Back				
				DwinMenu_main.set(MAIN_CASE_CONTROL);
				Draw_Main_Menu();
			break;

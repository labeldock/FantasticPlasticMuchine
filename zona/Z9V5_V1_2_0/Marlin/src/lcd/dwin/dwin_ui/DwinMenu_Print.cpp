...
void HMI_SelectFile() {
 ...
 if (encoder_diffState == ENCODER_DIFF_CW && fullCnt) {
  ...
  if (DwinMenu_file.now == 0) { // Back
   DwinMenu_main.set(MAIN_CASE_PRINT);
   Draw_Main_Menu();
  }
...
void Draw_Print_ProgressMixModel(){
  dwinLCD.Draw_Rectangle(1, COLOR_BG_BLACK, 10, 455, 250, 471);  
	if(mixer.gradient.enabled) {
		DWIN_Draw_MaskString_Default(10, 455, PSTR("Model: Gradient"));
		DWIN_Draw_IntValue_Default(2, 130, 455, mixer.gradient.start_vtool);
		DWIN_Draw_MaskString_Default(146, 455, PSTR("--->"));
		DWIN_Draw_IntValue_Default(2, 178, 455, mixer.gradient.end_vtool);
	}
	else if(mixer.random_mix.enabled) {
		DWIN_Draw_MaskString_Default(10, 455, PSTR("Model: Random"));
	}
	else{
...
void Popup_Window_ETempTooLow() {
	...
	DWIN_Draw_MaskString_Default_PopMenu((272 - 18*10), 235, PSTR("Nozzle is too cold"));
